<%@page import="com.moonBam.dto.MyScrapDTO" %>
<%@ page language="java" contentType="text/html; charset=UTF-8"
         pageEncoding="UTF-8" %>
<%
    MyScrapDTO sDTO = (MyScrapDTO) request.getAttribute("sDTO");
    int curPage = sDTO.getCurPage();
    int perPage = sDTO.getPerPage();
    int totalCount = sDTO.getTotalCount();

    int totalNum = totalCount / perPage;
    if (totalCount % perPage != 0) totalNum++;

    int perBlock = sDTO.getPerBlock();
    ...
    if (curPage == 1)
        ...
        out.print("<li class='page-item'><a class='page-link' href='scrap?curPage=1'>처음</a></li>");

    // 2. 이전 페이지로 이동
    if (curPage == 1)
        out.print("<li class='page-item'><a class='page-link'>&lt;</a></li>");
    else
        out.print("<li class='page-item'><a class='page-link' href='scrap?curPage=" + (curPage - 1) + "'>&lt;</a></li>");
    ...
    for (int i = startPage; i < endPage; i++) { // 마지막페이지번호는 표시 안 하고, for 루프 밑에서 표시( 공백 표시 때문에... )
        ...
        if (curPage == i)
            ...
            out.print("<li class='page-item'><a class='page-link' href='scrap?curPage=" + i + "'>" + i + "</a></li>");

    }
    if (curPage == endPage)
        out.print("<li class='page-item'><a class='page-link'>" + endPage + "</a></li>");
    else
        out.print("<li class='page-item'><a class='page-link'  href='scrap?curPage=" + endPage + "'>" + endPage + "</a></li>");
    ...
    if (curPage == totalNum)
        ...
        out.print("<li class='page-item'><a class='page-link' href='scrap?curPage=" + totalNum + "'>마지막</a></li>");
    out.print("</ul>");
