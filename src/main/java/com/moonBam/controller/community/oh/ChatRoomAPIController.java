 ...
 * 
 * @RequestMapping("/api/chatroom") public class ChatRoomAPIController {
 * 
 * private final ChatRoomsService chatRoomService;
 * 
 * // 채팅방 생성
 * 
 * @PostMapping public ResponseEntity<?> createChatRoom(@RequestBody
 * ChatRoomCreateRequestDTO requestDTO) { if(chatRoomService.create(requestDTO)
 * == 0) { return ResponseEntity.badRequest().body("채팅방 생성에 실패하셨습니다."); } return
 * ResponseEntity.ok().body("채팅방이 성공적으로 생성되었습니다."); }
 * 
 * 
 * // 채팅방 업데이트
 * 
